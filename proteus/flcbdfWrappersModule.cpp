...
CKSP_useTrueResidualConvergence(CKSP *self, PyObject *args, PyObject *kwds)
{
  ...
  PetscOptionsGetReal(PETSC_NULL,PETSC_NULL,&atol,&found);
  if (found) self->tres_ctx.atol  = atol;
  PetscOptionsGetReal(PETSC_NULL,PETSC_NULL,&rtol,&found);
  if (found) self->tres_ctx.rtol  = rtol;
  PetscOptionsGetReal(PETSC_NULL,PETSC_NULL,&divtol,&found);
  if (found) self->tres_ctx.divtol  = divtol;
  PetscOptionsGetInt(PETSC_NULL,PETSC_NULL,&max_it,&found);
