		...
		vel=&particle_velocities[i*sd_offset*nSpace];
	  	...
	  	w_s = vel[2];//particle_velocities[i*3+2];
	     	...
	     	//vel[0] = particle_velocities[i * sd_offset * nSpace + 0];
                ...
                double umag=sqrt(vel[0]*vel[0]+vel[1]*vel[1]+vel[2]*vel[2]);
                //    if (umag>0.05)
                      printf(".h file: solid ib velocity=%f\n",umag);
            ...
            double umag=sqrt(u_s*u_s+v_s*v_s+w_s*w_s);
            //if (umag>0.01)
              printf(".h file: solid ib vel=%f\n",umag);
